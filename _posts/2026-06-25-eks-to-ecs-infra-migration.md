...
## 마치며
마이그레이션 후 운영하면서 느낀 점은, 초기에는 리소스를 여유 있게 설정한 뒤 실제 트래픽과 사용량을 보면서 점진적으로 메모리나 CPU를 줄여나가는 방식이 더 적절했다는 것입니다.
저는 작은 용량에서 시작해 점차 늘려가는 방식으로 접근했는데, 이 과정에서 안정성 측면의 리스크가 있었다고 생각합니다. 처음부터 충분한 리소스를 확보한 상태로 운영했다면 더 안정적으로 대응할 수 있었을 것 같습니다.
